...
class SignUp extends Component {
	...
	onSubmit = async (e) => {
		...
		if (this.formValid(this.state)) {
			await axios
				.get('https://a2-ruize-nie.herokuapp.com/signup/' + email)
		...
		if (flag) {
			...
			axios
				.post('https://a2-ruize-nie.herokuapp.com/signup', user)
				...
				<section className="sign-content">
					<div className="sign-container">
						<form onSubmit={this.onSubmit} className="sign-form" noValidate>
							...
							{result.length > 0 && <span className="error-message">{result}</span>}
							<label htmlFor="username">Username</label>
							<input
								className={formError.username.length > 0 ? 'input-field error' : 'input-field'}
								type="text"
								name="username"
								id="username"
								value={username || ''}
							...
							<input
								className={formError.email.length > 0 ? 'input-field error' : 'input-field'}
								type="email"
								name="email"
								id="emial"
								value={email || ''}
